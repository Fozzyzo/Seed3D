...
bool RenderManager::startUp(RenderingSettings rendering_settings)
{
	...
	m_mesh = new Mesh();
	m_mesh->setPosition(1.0f, 1.0f, 1.0f);

	m_mesh2 = new Mesh();
	m_mesh2->setPosition(-3.0f, 1.0f, 1.0f);
	...
	if (!m_mesh->initialize(m_dx_api->getDxDevice(), m_dx_api->getDxDeviceContext(), (char*)"meshes/sphere.obj", (char*)"textures/metal_plate2_diffuse.tga", (char*)"textures/metal_plate2_normal.tga", (char*)"textures/metal_plate2_specular.tga"))
	{
		MessageBox(rendering_settings.window_handle, "Object initialization failed", "Error", MB_OK);
		return false;
	}

	if (!m_mesh2->initialize(m_dx_api->getDxDevice(), m_dx_api->getDxDeviceContext(), (char*)"meshes/cube.obj", (char*)"textures/wood_plank_diffuse.tga", (char*)"textures/wood_plank_normal.tga", (char*)"textures/wood_plank_specular.tga"))
	{
		MessageBox(rendering_settings.window_handle, "Object initialization failed", "Error", MB_OK);
		return false;
	}
...
void RenderManager::render()
{
	...
	m_shader_manager->renderShader(m_dx_api->getDxDeviceContext(), m_mesh->getIndexCount(), m_mesh->getPosition(), view_matrix, projection_matrix, 
	...
	m_mesh2->render(m_dx_api->getDxDeviceContext());
	m_shader_manager->renderShader(m_dx_api->getDxDeviceContext(), m_mesh2->getIndexCount(), m_mesh2->getPosition(), view_matrix, projection_matrix,
		m_mesh2->getMaterial(), m_light->getDirection(), m_light->getAmbientLight(), m_light->getDiffuseColor(), m_camera->getPosition(),
		m_light->getSpecularColor(), m_light->getSpecularStrength());
